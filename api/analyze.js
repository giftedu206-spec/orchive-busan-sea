...
{"name":"한국어 생물 이름","latin":"학명 또는 추정 학명","type":"위험 생물|해파리|연체동물|어류|기타","danger":1,"note":"사진 근거를 짧게 설명","confidence":70}`;
    ...
    return res.status(200).json({ name: String(parsed.name || '식별 중인 해양 생물'), latin: String(parsed.latin || '학명 확인 중'), type: types.includes(parsed.type) ? parsed.type : '기타', danger: Math.max(1, Math.min(5, Number(parsed.danger) || 1)), note: String(parsed.note || '사진을 바탕으로 분석했습니다.'), confidence: Math.max(0, Math.min(100, Number(parsed.confidence) || 70)) });
