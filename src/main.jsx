...
const beaches = [
  { name: '해운대', position: [35.1587, 129.1604] },
  { name: '광안리', position: [35.1532, 129.1186] },
  { name: '송정', position: [35.1788, 129.1995] },
  { name: '다대포', position: [35.0461, 128.9658] }
];
const getPlacePosition = (place = '') => beaches.find(beach => place.includes(beach.name))?.position || [35.1587, 129.1604];
...
const CreatureCard = ({item, compact=false}) => <article className={'creature-card '+(compact?'compact':'')}><img src={item.image} alt={item.name}/><div><div className="card-title">{item.danger>=4 && <b className="danger">위험</b>}{item.name}</div><small>{item.place} · {item.time}</small><Stars n={item.danger}/></div></article>;
function Header({title, back, action}) { return <header className="topbar">{back?<button className="plain" onClick={back}>‹</button>:<div className="brand"><span className="logo-mark"><img src={orchiveLogo} alt="Orchive 고래 로고"/></span><div>Orchive<small>부산 바다 생물 도감</small></div></div>}<h2>{title}</h2>{action&&<button className="plain">{action}</button>}</header> }
function Home({go, profile, discoveries}) { const level=Math.floor(profile.exp/300); const regions=new Set(discoveries.map(x=>x.place)).size; return <><Header/><section className="profile"><div className="avatar">{profile.avatar}<b>Lv. {level}</b></div><div><h3>{profile.name} ✎</h3><p>다음 레벨까지 <b>{300-(profile.exp%300)} EXP!</b></p><div className="progress"><i style={{width:(profile.exp%300)/3+'%'}}/></div><small>{profile.exp % 300} / 300 EXP</small></div><span>›</span></section><section className="badge-box"><div className="section-head"><b>내 배지 컬렉션</b><button onClick={()=>go('mypage')}>전체 보기 ›</button></div><div className="badges">{[['📷','첫 발견'],['📸','기록가'],['📍','탐험가'],['★','전문가']].map(([x,n],i)=><div className={discoveries.length===0&&i>0?'locked':''}><span>{x}</span><small>{n}</small></div>)}</div></section><div className="stats">{[[discoveries.length,'발견 기록'],[regions,'방문 지역'],[profile.exp,'누적 EXP']].map(([v,n])=><div><strong>{v}</strong><small>{n}</small></div>)}</div><div className="quick">{[['▣','발견 등록','register'],['●','발견 지도','map'],['▤','해양 생물 도감','book'],['!','위험 생물 안내','book']].map(([i,n,p])=><button onClick={()=>go(p)}><b>{i}</b><small>{n}</small></button>)}</div><section className="panel"><div className="section-head"><b>나의 최근 발견</b><button onClick={()=>go('book')}>도감 보기 ›</button></div>{discoveries.length?discoveries.slice(0,3).map(x=><CreatureCard item={x} compact/>):<div className="mini-empty">아직 등록된 발견이 없어요. 첫 생물을 기록해 보세요!</div>}</section></> }
function MapPage({discoveries}){const [filter,setFilter]=useState('전체');const types=['전체','위험 생물','해파리','연체동물','어류'];const shown=filter==='전체'?discoveries:discoveries.filter(x=>x.type===filter);return <><Header title="발견 지도" action="⌕"/><div className="chips">{types.map(t=><button className={filter===t?'active':''} onClick={()=>setFilter(t)}>{t}</button>)}</div><div className="map-wrap"><MapContainer center={[35.163,129.151]} zoom={13} minZoom={11} scrollWheelZoom className="real-map"><TileLayer attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/>{beaches.slice(0,3).map(beach=><CircleMarker key={beach.name} center={beach.position} radius={5} pathOptions={{color:'#0d73dd',fillColor:'#55c9ef',fillOpacity:1,weight:2}}><Tooltip permanent direction="top" offset={[0,-7]} className="beach-label">{beach.name} 해수욕장</Tooltip></CircleMarker>)}{shown.map(item=><CircleMarker key={item.id} center={getPlacePosition(item.place)} radius={item.danger>=4?15:12} pathOptions={{color:'#fff',fillColor:item.danger>=4?'#ef4b4b':'#146de1',fillOpacity:1,weight:3}}><Tooltip direction="top" offset={[0,-8]}>{item.emoji||'🐚'}</Tooltip><Popup><div className="map-popup"><b>{item.name}</b><small>{item.place}</small><Stars n={item.danger}/></div></Popup></CircleMarker>)}</MapContainer><p className="map-help">지도를 드래그하거나 손가락으로 벌리면 위치를 자세히 볼 수 있어요.</p></div><section className="panel recent"><div className="section-head"><b>최근 발견</b></div>{shown.length?shown.slice(0,3).map(x=><CreatureCard item={x} compact/>):<div className="mini-empty">아직 지도에 표시할 발견 기록이 없어요.</div>}</section></>}
function Register({go,setDraft}){const file=useRef();const [photo,setPhoto]=useState(null);const [imageData,setImageData]=useState(null);const [place,setPlace]=useState('해운대해수욕장');const [loading,setLoading]=useState(false);const [error,setError]=useState('');const choose=e=>{const f=e.target.files[0];if(!f)return;setPhoto(URL.createObjectURL(f));const reader=new FileReader();reader.onload=event=>setImageData(event.target.result);reader.readAsDataURL(f)};const startAnalysis=async()=>{if(!imageData){setError('먼저 생물 사진을 올려주세요.');return}setLoading(true);setError('');try{const apiBase=window.location.hostname.endsWith('github.io')?'https://orchive-busan-sea-web.vercel.app':'';const response=await fetch(`${apiBase}/api/analyze`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({imageData,place})});const result=await response.json();if(!response.ok)throw new Error(result.error||'AI 분석에 실패했습니다.');setDraft({photo:imageData,place,analysis:result,date:new Date().toISOString().slice(0,10)});go('analysis')}catch(err){setError(err.message)}finally{setLoading(false)}};return <><Header title="생물 발견 등록" back={()=>go('home')}/><main className="form-page"><button className="upload" onClick={()=>file.current.click()}>{photo?<img src={photo}/>:<><b>＋</b><span>발견한 생물 사진을 올려주세요</span><small>사진 선택하기</small></>}</button><input ref={file} type="file" accept="image/*" hidden onChange={choose}/><div className="ai-tip">✦ 사진을 올리면 실제 AI가 생물 이름과 위험도를 분석해 드려요.</div><label className="field">발견 위치<input type="text" placeholder="예: 해운대해수욕장" value={place} onChange={e=>setPlace(e.target.value)}/></label>{error&&<p className="form-error">{error}</p>}<button className="primary" disabled={loading} onClick={startAnalysis}>{loading?'AI가 사진을 분석하고 있어요…':'AI 분석 시작하기 ✦'}</button></main></>}
function Analysis({go,draft,onRegister}){const [confirmed,setConfirmed]=useState(false);const detected=draft.analysis;const isDanger=detected?.danger>=4;if(!detected)return <><Header title="AI 분석 결과" back={()=>go('register')}/><main className="analysis"><h2>분석 결과가 없습니다.</h2><button className="primary" onClick={()=>go('register')}>사진 다시 등록하기</button></main></>;return <><Header title="AI 분석 결과" back={()=>go('register')}/><main className="analysis"><img className="round-photo" src={draft.photo||creatures[0].image}/><h1>{detected.name}</h1><i>{detected.latin}</i><b className={isDanger?'danger':'ai-type'}>{detected.type}</b><p>AI 분석 위험도 <Stars n={detected.danger}/></p><div className="confidence"><small>AI 분석 신뢰도</small><strong>{detected.confidence}%</strong><div className="progress"><i style={{width:detected.confidence+'%'}}/></div></div><div className="feature"><b>AI가 발견한 특징</b><p>✓ {detected.note}</p><p>✓ 발견 위치와 사진 정보를 함께 기록해요</p><p>✓ 시민 발견 데이터로 도감에 추가됩니다</p></div><div className="ai-confirm"><b>AI 분석 결과가 맞나요?</b><p>시민 확인 기록은 도감의 신뢰도를 높여요.</p><div><button className={confirmed?'confirm-selected':''} onClick={()=>setConfirmed(true)}>✓ 맞아요</button><button onClick={()=>go('register')}>↻ 다른 생물이에요</button></div></div>{isDanger&&<div className="warning"><b>🛡 주의하세요!</b><p>AI가 위험 가능성이 있는 생물로 분류했어요. 절대 만지지 말고 주변에 알려주세요.</p></div>}<button className="primary" disabled={!confirmed} onClick={onRegister}>{confirmed?'이 정보로 등록하기':'먼저 AI 결과를 확인해 주세요'}</button></main></>}
...
function MyPage({profile,setProfile,discoveries}){const [editing,setEditing]=useState(false);const level=Math.floor(profile.exp/300);const regions=new Set(discoveries.map(x=>x.place)).size;return <><Header title="마이페이지" action="⚙"/><main className="my"><div className="myhero"><div className="avatar">{profile.avatar}<b>Lv. {level}</b></div>{editing?<input className="name-input" value={profile.name} maxLength="12" onChange={e=>setProfile({...profile,name:e.target.value})}/>:<h2>{profile.name}</h2>}<p>부산 바다를 기록하는 시민 과학자</p><div className="progress"><i style={{width:(profile.exp%300)/3+'%'}}/></div><small>{profile.exp%300} / 300 EXP · 다음 레벨까지 {300-(profile.exp%300)} EXP</small><button className="edit-profile" onClick={()=>setEditing(!editing)}>{editing?'완료':'프로필 수정'}</button>{editing&&<div className="avatar-picker"><small>프로필 아이콘 선택</small><div>{['🌊','🐬','🐳','🐠','🪼','🐚'].map(icon=><button className={profile.avatar===icon?'chosen':''} onClick={()=>setProfile({...profile,avatar:icon})}>{icon}</button>)}</div></div>}</div><div className="stats">{[[discoveries.length,'발견'],[regions,'지역'],[level,'레벨']].map(([v,n])=><div><strong>{v}</strong><small>{n}</small></div>)}</div><section className="panel"><b>나의 획득 배지</b><div className="badges white">{[['📷','첫 발견'],['📸','기록왕'],['📍','탐험가'],['★','전문가']].map(([x,n],i)=><div className={discoveries.length===0&&i>0?'locked':''}><span>{x}</span><small>{n}</small></div>)}</div></section><section className="panel"><b>EXP 획득 방법</b><p className="xp">첫 발견 <strong>+200 EXP</strong></p><p className="xp">두 번째부터 발견 등록 <strong>+100 EXP</strong></p><p className="xp">새로운 지역 발견 <strong>+50 EXP</strong></p><p className="xp">희귀 생물 발견 <strong>+200 EXP</strong></p></section></main></>}
function App(){
  ...
  const registerDiscovery=async()=>{
    ...
    const item={id:Date.now(),name,latin:analysis.latin,type:analysis.type,emoji:analysis.emoji||'🐚',image:savedPhoto,danger:analysis.danger,count:1,place:draft.place||'해운대해수욕장',time:'방금 전',verification:'시민 확인'};
  ...
  let content={home:<Home go={setPage} profile={profile} discoveries={discoveries}/>,map:<MapPage discoveries={firebaseEnabled?publicDiscoveries:discoveries}/>,register:<Register go={setPage} setDraft={setDraft}/>,analysis:<Analysis go={setPage} draft={draft} onRegister={registerDiscovery}/>,complete:<Complete go={setPage} draft={draft}/>,book:<Book discoveries={discoveries} likes={likes} setLikes={setLikes} comments={comments} setComments={setComments} profile={profile}/>,mypage:<MyPage profile={profile} setProfile={setProfile} discoveries={discoveries}/>}[page];
  return <div className="app"><div className="shell">{content}{!['analysis','complete'].includes(page)&&<nav>{navs.map(([p,i,n])=><button className={page===p?'selected':''} onClick={()=>setPage(p)}><span>{i}</span><small>{n}</small></button>)}</nav>}</div></div>
};
